...
def analyze_field_text(text, field_name, min_length=50, keywords=None):
    ...
    # Если поле пустое или слишком короткое
    if not text or len(text.strip()) < 10:
        return {
            'status': 'error',
            'score': 0,  # ← 0 баллов за пустое поле!
    ...
    # Если текст есть, но слишком короткий
    if len(text.strip()) < min_length:
        return {
            'status': 'warning',
            'score': max(0, min(100, (len(text) / min_length) * 50)),  # Максимум 50% за короткий текст
    ...
    # Нормальная обработка заполненного поля
    ...
    # Проверка ключевых слов (соответствие теме)
    keyword_score = 100
    missing_keywords = []
    if keywords:
...
def check_financial_data(amount, team_size=None):
    ...
    try:
        ...
        if amount <= 0:
            ...
            errors.append('Сумма превышает максимальный лимит гранта (10 млн рублей)')
...
def ai_validate_application_detailed(data):
    ...
    # Определяем ключевые слова для разных конкурсов
    contest = data.get('contest', '')

    # Ключевые слова для разных конкурсов
    keywords_by_contest = {
        'Культурный код': {
            'summary': ['культур', 'искусств', 'традиц', 'наследи', 'творчеств', 'фестивал', 'выставк'],
            'problem': ['культур', 'доступн', 'сохранен', 'развити', 'просвещен'],
            'uniqueness': ['инновац', 'уникальн', 'новизн', 'оригинальн', 'впервые'],
            'audience': ['зрител', 'посетител', 'участник', 'жител', 'населен'],
            'results': ['охват', 'количеств', 'посещаемост', 'удовлетворен', 'вовлечен']
        },
        'Социальное действие': {
            'summary': ['социальн', 'помощ', 'поддержк', 'волонтер', 'благотворител', 'общественн'],
            'problem': ['социальн', 'проблем', 'нужда', 'уязвим', 'малообеспечен', 'инвалид', 'пенсионер'],
            'uniqueness': ['новый подход', 'эффективн', 'результативн', 'методик'],
            'audience': ['семь', 'дет', 'молодеж', 'пенсионер', 'инвалид', 'волонтер'],
            'results': ['помощ', 'улучшен', 'качеств', 'охват', 'количеств']
        }
    }

    keywords = keywords_by_contest.get(contest, {})

    # 1. Анализ текстовых полей - ВСЕ поля участвуют в расчёте
    text_fields = {
        'summary': {'weight': 3, 'name': 'Краткое описание проекта', 'min_length': 100,
                    'keywords': keywords.get('summary', [])},
        'problem': {'weight': 3, 'name': 'Описание проблемы', 'min_length': 100,
                    'keywords': keywords.get('problem', [])},
        'uniqueness': {'weight': 2, 'name': 'Уникальность решения', 'min_length': 80,
                       'keywords': keywords.get('uniqueness', [])},
        'plan': {'weight': 2, 'name': 'План реализации', 'min_length': 100, 'keywords': []},
        'results': {'weight': 2, 'name': 'Ожидаемые результаты', 'min_length': 80,
                    'keywords': keywords.get('results', [])},
        'audience': {'weight': 1, 'name': 'Целевая аудитория', 'min_length': 50,
                     'keywords': keywords.get('audience', [])}
    }
    ...
    for field_key, field_info in text_fields.items():
        ...
        # ВСЕ поля участвуют в расчёте с их весом
    ...
    # Нормализуем балл текстовых полей
    if total_weight > 0:
    ...
    if direction:
        ...
        direction_score = 0  # ← МЕНЯЕМ на 0!
        field_results['direction'] = {'status': 'error', 'score': 0, 'message': 'Направление не указано'}
        result['recommendations'].append('Укажите направление вашего проекта')
    ...
    if data.get('contest'):
        contest_score = 100
        field_results['contest'] = {'status': 'valid', 'score': 100, 'message': f'Конкурс: {data.get("contest")}'}
    else:
        contest_score = 0  # ← МЕНЯЕМ на 0!
        field_results['contest'] = {'status': 'error', 'score': 0, 'message': 'Конкурс не выбран'}
        result['recommendations'].append('Выберите конкурс для участия')

    # ===== РАСЧЁТ ИТОГОВОГО БАЛЛА =====
    # Все поля участвуют с весами
    all_weights = {
        'text_fields': 13,  # сумма весов текстовых полей (3+3+2+2+2+1)
        'financial': 2,
        'direction': 2,
        'team': 1,
        'contest': 1
    }
    total_weight_all = sum(all_weights.values())  # = 19

    # Взвешенная сумма баллов
    weighted_sum = (
            text_score * all_weights['text_fields'] +
            financial_score * all_weights['financial'] +
            direction_score * all_weights['direction'] +
            team_score * all_weights['team'] +
            contest_score * all_weights['contest']
    )

    result['total_score'] = round(weighted_sum / total_weight_all, 2)
    ...
    # Добавляем детализацию баллов для отладки
...
# Остальные функции (generate_hearing_schedule, detect_anomalies, generate_final_report, auto_assign_experts)
def generate_hearing_schedule(applications):
...
def generate_final_report(applications):
